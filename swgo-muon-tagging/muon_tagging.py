...
class MuTagRise(MuTagLR):
    ...
    def muonScore(self, taggedPmtEvents):
        return muonScoreLR(
            self.xedges,
            self.yedges,
            taggedPmtEvents["per" + self.config["high"] + "Upper"]
            - taggedPmtEvents["per" + self.config["low"] + "Upper"],
            taggedPmtEvents["per" + self.config["high"] + "Lower"]
            - taggedPmtEvents["per" + self.config["low"] + "Lower"],
            self.histLR,
        )
...
class MuTagChargeRise(MuonTagger):
    ...
    def muonScore(self, taggedPmtEvents):
        scoreCT = muonScoreCT(
            taggedPmtEvents, self.xtedges, self.ytedges, self.low, self.high, self.xcgrid, self.ycgrid, self.rtHists
        )
        scoreCT *= muonScoreLR(
            self.xcedges, self.ycedges, taggedPmtEvents["upper"], taggedPmtEvents["lower"], self.histLR
        )
        return scoreCT
