...
for batch, report in uproot.iterate(
    ...
    for i in np.arange(ak.num(peTimes, 0)):
        ...
        primaries.append(
            np.array(
                [(i + report.start, batch["HAWCSim.Evt.pType"][i], batch["HAWCSim.Evt.Energy"][i])],
                dtype=TYPE_PRIMARIES,
            )
        )
        # skip if empty (this actually happens!)
        if pets.size == 0 or ~args.events:
            continue
        # count upper/lower
        ...
        np.bitwise_or.at(
            tagsUpper,
            (pmtIds[pmtIds % 2 == 0] / 2).astype(int),
            (pis < 4) * TAG_E
            | (np.logical_and(5 <= pis, pis < 7)) * TAG_MU
            | (np.logical_and(7 <= pis, pis < 13)) * TAG_MESON
            | (pis >= 13) * TAG_OTHER,
        )
        ...
        np.bitwise_or.at(
            tagsLower,
            (pmtIds[pmtIds % 2 == 1] / 2).astype(int),
            (pis < 4) * TAG_E
            | (np.logical_and(5 <= pis, pis < 7)) * TAG_MU
            | (np.logical_and(7 <= pis, pis < 13)) * TAG_MESON
            | (pis >= 13) * TAG_OTHER,
