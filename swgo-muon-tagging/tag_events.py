...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--name", help="name of tagger (including directory name)", default="models/mu_tag_LR_charge"
    )
    ...
    parser.add_argument(
        "--sroc", action="store_true", help="plot ROC curves for showers, only use with -b and -c (EDA)"
    )
    parser.add_argument(
        "-c", "--cuts", type=float, default=None, help="muon score cuts for EDA", nargs="+", metavar="C"
    )
    ...
    if args.analysis:
        ...
        cnts, tCnts, ids = tagShowers(
            mt, taggedPmtEvts, cut=cuts, truth=True, ratio=False, makeIds=True, proportion=True
        )
        selP = primaries["showerType"][ids] == ID_PROTON
        selG = primaries["showerType"][ids] == ID_PHOTON
        energyDependentAnalysis(
            cnts, tCnts, cuts, sep, ids, primaries, args.sroc, args.bins, args.profiles, args.percentiles
        )
