...
parTicks = (
    ...
    ["$e^+$", "$e^-$", "$\mu^+$", "$\mu^-$", "$\pi^+$", "$\pi^-$", "$K^+$", "$K^-$", "$p$", "$\\bar p$"],
)
cdx = 0
for pmtIds, peTimes, ft, ex, ey, parPType in zip(
    events["HAWCSim.PE.PMTID"],
    events["HAWCSim.PE.Time"],
    events["HAWCSim.Evt.firstTime"],
    events["HAWCSim.Evt.X"],
    events["HAWCSim.Evt.Y"],
    events["HAWCSim.PE.parPType"],
):
    # plot all
    ...
    if makePlots:
        ...
        plt.scatter(east / 100, north / 100, c="grey")
        plt.scatter(
            east[(pmtIdP / 2).astype(int)] / 100,
            north[(pmtIdP / 2).astype(int)] / 100,
            vmin=cntPL.min(),
            vmax=cntPL.max(),
            c=cntPL,
            cmap=cm,
    ...
    taggedPmtEvts.append(
        (cdx * np.ones(pmtIdG[sel].shape), pmtIdG[sel], upper[sel], lower[sel], tagsUpper[sel], tagsLower[sel])
    )
    cdx += 1
    # if cdx > 5: break
    plt.show()
