...
if __name__ == "__main__":
    ...
    if plotHists:
        for q, l in zip(
            [diffUpper, diffLower],  # per10Upper, per10Lower, per90Upper, per90Lower,
            [
                "difference upper",
                "difference lower",
            ],
        ):  # "0.1 upper", "0.1 lower","0.9 upper", "0.9 lower",
    ...
    hist, xedges, yedges = np.histogram2d(
        np.clip(diffUpper, bins[0], bins[-1]), np.clip(diffLower, bins[0], bins[-1]), bins=(bins, bins)
    )
    if plotHists:
        plotLogHist2d(
            xedges,
            yedges,
            hist,
            title="Rise time histogram",
            xlabel="upper 10-" + p + " time/ns",
            ylabel="lower 10-" + p + " time/ns",
        )

    # electron/muon histograms
    *_, muAny, histEOnly, histMuAny, histLR = makeHistograms(
        xedges,
        yedges,
        taggedPmtEvts,
        np.clip(diffUpper, bins[0], bins[-1]),
        np.clip(diffLower, bins[0], bins[-1]),
        clip=False,
    )
    for title, hist in zip(["Only electrons", "Any muons", "Likelihood ratio"], [histEOnly, histMuAny, histLR]):
        if plotHists:
            plotLogHist2d(xedges, yedges, hist, title, "upper 10-" + p + " time/ns", "lower 10-" + p + " time/ns")
