...
def tagShowers(muonTagger, taggedPmtEvts, cut=1, truth=False, ratio=False, makeIds=False, proportion=False):
    ...
    if not isinstance(cut, np.ndarray):
        cnts = cnts[0]
    if not truth:
        return (cnts, cdx[idx]) if makeIds else cnts
    # sum true muons
    eOnly, muAny = getEMuTags(taggedPmtEvts)
...
def plotRatioEMu(taggedPmtEvts, primaries, plotProfiles=False):
    ...
    if plotProfiles:
        plt.errorbar(
            *profilePoints(primaries["showerEnergy"][ids][selP] / 1000, muCnt[selP] / eCnt[selP]), label="Protons"
        )
        plt.errorbar(
            *profilePoints(primaries["showerEnergy"][ids][selG] / 1000, muCnt[selG] / eCnt[selG]), label="Gammas"
        )
    else:
...
def energyDependentAnalysis(
    cnts, tCnts, cuts, sep, ids, primaries, plotEdst=True, eBinCnt=4, plotProfiles=True, plotPercentiles=False
):
    """
    ...
    for minE, maxE, i in zip(energyBins[:-1], energyBins[1:], np.arange(snrBest.size)):
        ...
        snr = plotSeparationCuts(
            np.array(["{:.1f}@{:.0f}-{:.0f}TeV".format(c, minE / 1000, maxE / 1000) for c in cuts]),
            cntsPed,
            cntsGed,
            sep=sep,
            plot=plotEdst,
