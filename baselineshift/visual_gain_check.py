...
br_space = np.logspace(5,10,num=20)
...
for times in range(30):
	...
	for br in br_space:
		...
		nb_samp = esim.no_signal_duration/esim.t_sample
...
for i in range(len(br_space)):
	###Calculer la standard deviation for each background
	numerical_deviations.append(np.std([x[i] for x in all_lines]))
	means.append(np.mean([x[i] for x in all_lines]))
...
plt.fill_between(br_space, np.subtract(means,numerical_deviations), np.add(means,numerical_deviations), color=tr_color)
for line in all_lines:
	plt.semilogx(br_space, line, label="True")

plt.semilogx(br_space, np.subtract(means,gain_theoretical_current_std), color="red")
plt.semilogx(br_space, np.add(means,gain_theoretical_current_std), color="red")
